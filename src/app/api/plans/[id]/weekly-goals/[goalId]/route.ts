...
export async function PUT(
  request: NextRequest,
  context: { params: { id: string; goalId: string } }
) {
  try {
    ...
    if (!goalId || isNaN(parseInt(goalId))) {
      return NextResponse.json(
        { error: 'Valid goal ID is required', code: 'INVALID_GOAL_ID' },
        { status: 400 }
      );
    }
    ...
    const goalRecord = await db
      ...
      .where(eq(weeklyGoals.id, parseInt(goalId)))
      .limit(1);

    if (goalRecord.length === 0) {
      return NextResponse.json(
        { error: 'Weekly goal not found', code: 'GOAL_NOT_FOUND' },
        { status: 404 }
      );
    }
    ...
    if (goalRecord[0].userId !== userId) {
      return NextResponse.json(
        { error: 'You do not have permission to update this weekly goal', code: 'FORBIDDEN' },
        { status: 403 }
      );
    }

    if (goalRecord[0].studyPlanId !== parseInt(id)) {
      return NextResponse.json(
        { error: 'Goal does not belong to the specified study plan', code: 'PLAN_MISMATCH' },
        { status: 400 }
      );
    }
